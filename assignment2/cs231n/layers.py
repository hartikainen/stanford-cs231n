...
def affine_backward(dout, cache):
  ...
  dw = np.dot(x.reshape(dout.shape[0], -1).T, dout).reshape(w.shape)
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    y_ = gamma * x_ + beta
    out = y_
    cache = (x, x_, gamma, running_mean, running_var, eps)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  else:
...
def batchnorm_backward(dout, cache):
  ...
  dx_= dout * gamma
  dvar  = -0.5 * np.sum(
    dx_ * (x-mean) * np.power(var + eps, -3.0/2.0),
    axis=0
  )
  dmean = (
    np.sum(dx_ * -1.0 / np.sqrt(var + eps), axis=0) +
    dvar * np.sum(-2.0 * (x-mean), axis=0) / m
  )

  dx = dx_ * 1.0 / np.sqrt(var + eps) + dvar * 2.0 * (x - mean) / m + dmean / m
...
def batchnorm_backward_alt(dout, cache):
  ...
  dx = (
    (gamma * np.power(var + eps, -0.5) / m) *
    (
      dout * m -
      np.sum(dout, axis=0) -
      (x-mean) * np.power(var + eps, -1.0) * np.sum(dout * (x-mean), axis=0)
    )
  )
  dgamma = np.sum(x_*dout, axis=0)
  dbeta = np.sum(dout, axis=0)
